...
namespace nasedkin_e_strassen_algorithm_mpi {
    ...
    void run_strassen_test_for_matrix_size(size_t matrix_size) {
        ...
        if (world.rank() == 0) {
            double residual_parallel = calculate_residual(A_flat, B_flat, C_parallel, matrix_size);
            double residual_sequential = calculate_residual(A_flat, B_flat, C_sequential, matrix_size);

            ASSERT_LT(residual_parallel, 1e-3) << "Parallel solution did not match the sequential solution within tolerance.";
            ASSERT_LT(residual_sequential, 1e-3) << "Sequential solution did not match the expected result within tolerance.";
        } else {
