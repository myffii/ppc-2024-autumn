...
void run_strassen_test_for_matrix_size(size_t matrix_size) {
  ...
  StrassenAlgorithmParallel strassen_parallel(taskDataPar);
  ...
  if (world.rank() == 0) {
    ...
    StrassenAlgorithmSequential strassen_sequential(taskDataSeq);
    ...
    for (size_t i = 0; i < matrix_size * matrix_size; ++i) {
      ASSERT_NEAR(C_parallel[i], C_sequential[i], 1e-6);
    }
  }
}
