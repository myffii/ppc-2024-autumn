...
void StrassenAlgorithmMPI::split_matrix_for_processes(const std::vector<std::vector<double>>& matrix,
  ...
  int n = matrix.size();
  int block_size = n / std::sqrt(num_processes);
  int row_start = (rank / std::sqrt(num_processes)) * block_size;
  int col_start = (rank % (int)std::sqrt(num_processes)) * block_size;

  localMatrix.resize(block_size, std::vector<double>(block_size));
